...
r.put('/types/:id', (req, res) => {
  ...
  ).run(merk, materiaal, inkoop_prijs_per_kg, dichtheid_g_per_cm3||1.24, leverancier||null, notities||null, req.params.id);
  res.json({ ok: true });
});
...
r.put('/rollen/:id', (req, res) => {
  const db = getDb();
  const { gewicht_gram_huidig, kleur, locatie, actief } = req.body;
  db.prepare('UPDATE filament_rollen SET gewicht_gram_huidig=?,kleur=?,locatie=?,actief=? WHERE id=?')
    .run(gewicht_gram_huidig, kleur||null, locatie||null, actief?1:0, req.params.id);
  res.json({ ok: true });
});
