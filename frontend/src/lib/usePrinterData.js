  ...
  useEffect(() => {
    ...
    async function poll() {
      ...
      for (const p of printerConfig) {
        ...
        // Kobra heeft een aparte, enkel-voor-foutdetectie-bedoelde status
        // (fail_status = print_state) naast de stabiele hoofdstatus (status =
        // printer_state, enkel busy/free — zie printers.js). Voor Bambu/Ender
        // bestaat deze entity niet, dan blijft dit gewoon leeg.
        const failStatusLower = (s.fail_status || '').toLowerCase();
        const isActief = ['running','printing','prepare','busy'].includes(statusLower);
        ...
        const isDone   = ['finish','finished','complete','success','done','free'].includes(statusLower);
        // Failed/cancelled kan van de hoofdstatus komen (Bambu/Ender), of van
        // de aparte fail_status-entity (Kobra — printer_state kent zelf geen
        // fail/cancel-waarde, enkel busy/free).
        const isFailed = ['failed','cancelled'].includes(statusLower) || ['failed','cancelled'].includes(failStatusLower);
