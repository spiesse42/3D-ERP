...
function TypeModal({ type, onClose, onSaved }) {
  const [form, setForm] = useState(type || { merk: '', materiaal: 'PLA+', kleur: '', inkoop_prijs_per_kg: '', dichtheid_g_per_cm3: 1.24, leverancier: '' });
...
function RolModal({ types, onClose, onSaved }) {
  const [form, setForm] = useState({ filament_type_id: types[0]?.id || '', gewicht_gram_start: 1000, locatie: '', gekocht_op: new Date().toISOString().split('T')[0] });
  const set = (k, v) => setForm(f => ({ ...f, [k]: v }));
  async function save() {
    try {
      await api.post('/filament/rollen', form);
      onSaved();
    } catch (e) { alert(e.message); }
  }
  return (
      ...
      <div className="modal">
        ...
        <div className="form-group">
          ...
          <select value={form.filament_type_id} onChange={e => set('filament_type_id', e.target.value)}>
            {types.map(t => <option key={t.id} value={t.id}>{t.merk} {t.materiaal} {t.kleur}</option>)}
          </select>
        </div>
        <div className="form-row">
    ...
    <div>
      ...
      {tab === 'rollen' && (
        ...
        <div className="card" style={{ padding: 0 }}>
          <table>
            <thead><tr><th>Type</th><th>Huidig gewicht</th><th>Restwaarde</th><th>Locatie</th><th>Status</th><th>Acties</th></tr></thead>
            <tbody>
              {rollen.map(r => (
                <tr key={r.id} style={{ opacity: r.actief ? 1 : 0.5 }}>
                  <td>
                    <div style={{ fontWeight: 500 }}>{r.merk} {r.materiaal}</div>
                    <div style={{ fontSize: 11, color: 'var(--muted)' }}>{r.kleur}</div>
                  </td>
                  <td>
      ...
      {tab === 'types' && (
        ...
        <div className="card" style={{ padding: 0 }}>
          <table>
            <thead><tr><th>Merk</th><th>Materiaal</th><th>Kleur</th><th>Prijs/kg</th><th>Dichtheid</th><th>Leverancier</th><th>Acties</th></tr></thead>
            <tbody>
              {types.map(t => (
                <tr key={t.id}>
